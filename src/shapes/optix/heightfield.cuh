...
extern "C" __global__ void __intersection__heightfield() {
    ...
    Vector2f local_min_target_bounds[2] = { -1.0f + (float)row_offset * cell_size[0], -1.0f + (float)row_nr * cell_size[1] };
    Vector2f local_max_target_bounds[2] = { -1.0f + (float)(row_offset + 1) * cell_size[0], -1.0f + (float)(row_nr + 1) * cell_size[1] };
    ...
    float max_displacement_in_tile = fmaxf(heightfield->grid_data[left_bottom_idx], 
                                ...
                                fmaxf(heightfield->grid_data[right_top_idx]))));
